...
void printTemp()
{
    ...
    For(i,0,N)
    {
        cout<<tempSuffixes[i].index<<"\t"<<"\t"<<tempSuffixes[i].rank[0]<<"\t"<<tempSuffixes[i].rank[1]<<"\n";
    }
}
...
string listOfSuffixes[200005];
void buildSuffixArray()
{
    int i,j;
    //cout<<"buildSuffixArray called for "<<str<<"\n";
    for(i=0;i<N;i++)
    {
        P[0][i]=str[i]-'A';
        suffixes[i].index=i;
...
void findPassword()
{
    ...
    for(i=0;i<N;i++)
    {
        ...
        if(listOfSuffixes[suffixes[i].index].length()>=(N) and suffixes[i].index<(N))
        {
